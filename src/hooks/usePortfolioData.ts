...
const PORTFOLIO_CACHE_KEY = 'portfolioData';
...
function getInitialState(): PortfolioState {
  ...
  try {
    ...
    console.error('Failed to parse portfolio cache:', error);
  }
...
function persistToCache(state: PortfolioState) {
  if (typeof window !== 'undefined') {
    try {
      localStorage.setItem(PORTFOLIO_CACHE_KEY, JSON.stringify(state));
    } catch (error) {
...
export function usePortfolioData(userId: string | undefined) {
  ...
  // Fetch data when userId available
  useEffect(() => {
    if (!userId) return;
    fetchAllData();
  }, [userId, fetchAllData]);
